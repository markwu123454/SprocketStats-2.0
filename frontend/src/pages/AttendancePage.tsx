...
export default function AttendancePage() {
    ...
    // Default clock in/out to that day's scheduled meeting, so the wheels start
    // right rather than always at the same fallback time. Only overrides them
    // when the selected date actually has a meeting; picking an unscheduled date
    // leaves whatever the wheels were already set to.
    useEffect(() => {
        const meeting = scheduledDates.find(([key]) => key === selectedDateKey)?.[1]
        if (!meeting) return
        setClockIn(toHHMM(new Date(meeting.start_time)))
        setClockOut(toHHMM(new Date(meeting.end_time)))
        setSubmitted(false)
    }, [selectedDateKey, scheduledDates])
                ...
                <div className="flex flex-col gap-4">
                    ...
                    <div className="rounded-xl border p-5 flex flex-col backdrop-blur-sm" style={cardStyle}>
                        <div className="flex items-center justify-between mb-2">
                            <h3 className="font-bold theme-text">Upcoming events</h3>
                            <span className="text-xs theme-subtext-color">{weekRangeLabel()}</span>
                        </div>
                        {loading ? (
                            <p className="text-sm theme-subtext-color py-2">Loading…</p>
                        ) : meetings.length === 0 ? (
                            <p className="text-sm theme-subtext-color py-2">No meetings scheduled yet.</p>
                        ) : (
